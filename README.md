...
5. Средний риск получает усложненную капчу с отдельным лимитом попыток.
6. Низкий риск получает обычную капчу.
7. После верной капчи бот автоматически одобряет заявку в канал.
...
- Средний риск: бот выдает усложненную капчу с отдельным лимитом попыток.
- Низкий риск: бот выдает обычную капчу.
